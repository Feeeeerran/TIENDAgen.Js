...
function Rellenar(prodLista) {
	...
	prodLista.forEach( producto=>{
		...
		$(`#mother${i}>div`).append(
			...
			`<div class="d-flex justify-content-between"><p class="card-text albumSection_producto--price">${prodLista[i].precio}</p><p class="albumSection_producto--button" id="${i}">Comprar</p></div>`);
		i++;
	});
};
